...
function getStyles(accent: string, line: string) {
...
export default function ClassicoTemplate({ curriculo, corDestaque }: { curriculo: Curriculo; corDestaque: string }) {
  const c = curriculo;
  const accent = corDestaque;
  const line = lighten(accent, 0.6);
  const styles = getStyles(accent, line);
